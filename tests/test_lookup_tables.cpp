...
//   - sin_lut(float) vs std::sin:  <= 5e-3 (5 x 10^-3)
...
// ---------------------------------------------------------------------------
// AC-L05: sin_q31[256] == INT32_MAX +/- 1 LSB  (sin(pi/2) ~ 1.0)
// Index 256 corresponds to pi/2 in a 1024-entry full-period table.
...
// ---------------------------------------------------------------------------
// AC-L06: sin_q31[512] == 0 +/- 1 LSB  (sin(pi) ~ 0)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// AC-L07: sin_q31[768] == -INT32_MAX +/- 1 LSB  (sin(3pi/2) ~ -1.0)
...
// ---------------------------------------------------------------------------
// AC-L09: sin_lut(pi/2) ~ 1.0f  (within 5e-3)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// AC-L12: cos_lut(pi/2) ~ 0.0f  (within 5e-3)
// cos(pi/2) = 0
// ---------------------------------------------------------------------------
...
// sqrt0.25 = 0.5 exactly in real arithmetic
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// AC-L17: arctan_lut(1.0f) ~ pi/4  (arctan(1) = pi/4 ~ 0.7854)
// ---------------------------------------------------------------------------
...
// arctan(127/128) ~ 0.7768 rad; scaled to Q31 of (value / (pi/4)):
//   q31 = round(arctan(127/128) / (pi/4) * INT32_MAX)
...
TEST_CASE("AC-L20: arctan_q31[127] and wrapper are consistent at index 127", "[core][lookup_tables]") {
    ...
    // If the table stores Q31 of (arctan(i/128) / (pi/4)), then at index 127:
    //   expected_q31 ~ round(arctan(127/128) / (pi/4) * INT32_MAX)
